...
class classeLoginDAO {
    ...
    public function fazerLogin($cpforemail, $senha) {
        try {
            $pdo = conexao::getInstance();
            //pesquisa nas duas tabelas
            $sql = "SELECT idCliente,email,nome,senha,perfil FROM cliente where email=:cpforemail   and senha=:senha or cpf=:cpforemail and
senha=:senha
    UNION
    SELECT idFuncionario,email,nome,senha,perfil from funcionario where email=:cpforemail   and senha=:senha or cpf=:cpforemail and
senha=:senha    ";
            $smtp = $pdo->prepare($sql);
            $smtp->bindValue(':cpforemail', $cpforemail);
...
//            echo "<script type='text/javascript'>alert('Voce foi desconectado!');
//                location.href='index.php';</script>";
              header('location:index.php?msg=erro'); 
            $_SESSION['msg'] = 'Voce foi desconectado!';
